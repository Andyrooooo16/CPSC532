...
export async function renderPdf({ container, url, enableAnnotations }) {
  // Load the PDF
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    // If enableAnnotations, render annotations
    if (enableAnnotations) {
      // Render annotations layer
      // This is simplified; full implementation would handle different annotation types
      const annotations = await page.getAnnotations();
      for (const annotation of annotations) {
        if (annotation.subtype === 'Link') {
          // Handle links, perhaps add hover popups
          // For simplicity, add a div with popup on hover
          const linkDiv = document.createElement('div');
          linkDiv.style.position = 'absolute';
          linkDiv.style.left = annotation.rect[0] * scale + 'px';
          linkDiv.style.top = (viewport.height - annotation.rect[3] * scale) + 'px';
          linkDiv.style.width = (annotation.rect[2] - annotation.rect[0]) * scale + 'px';
          linkDiv.style.height = (annotation.rect[3] - annotation.rect[1]) * scale + 'px';
          linkDiv.style.backgroundColor = 'rgba(255, 255, 0, 0.3)';
          linkDiv.title = annotation.contents || 'Citation'; // Hover popup
